...
class Generator:
    ...
    def __init__(self, release):
        ...
        if self._generate_addons_file(addons_xml_path):
            print(
                "Successfully updated " + addons_xml_path
            )

            if self._generate_md5_file(addons_xml_path, md5_path):
                print("Successfully updated "+ md5_path)
            ...
            for fn in filenames:
                if fn.lower().endswith("pyo") or fn.lower().endswith("pyc"):
                    ...
                    try:
                        os.remove(compiled)
                        print(
                            "Removed compiled python file: "+ compiled
                        )
                    except OSError:
                        print(
                            "Failed to remove compiled python file: "+compiled
                        )
            for dirc in dirnames:
                if "pycache" in dirc.lower():
                    compiled = os.path.join(parent, dirc)
                    try:
                        shutil.rmtree(compiled)
                        print(
                            "Removed __pycache__ cache folder: "+compiled
                        )
                    except OSError:
                        print(
                            "Failed to remove __pycache__ cache folder: "+compiled
                        )
    ...
    def _create_zip(self, folder, addon_id, version):
        ...
        final_zip = os.path.join(
            zip_folder, "{0}-{1}.zip".format(addon_id, version))
        if not os.path.exists(final_zip):
            ...
            for root, dirs, files in os.walk(addon_folder):
                ...
                for f in files:
                    ...
                    local_zip.write(fullpath, archive_name, zipfile.ZIP_DEFLATED)

            local_zip.close()
            size = convert_bytes(os.path.getsize(final_zip))
            print(
                "Zip created for "+ addon_id + " "+version+" "+size
            )
    ...
    def _copy_meta_files(self, addon_id, addon_folder):
        ...
        tree = ElementTree.parse(os.path.join(
            self.release_path, addon_id, "addon.xml"))
        root = tree.getroot()
    ...
    def _generate_addons_file(self, addons_xml_path):
        ...
        for addon in folders:
            try:
                addon_xml_path = os.path.join(
                    self.release_path, addon, "addon.xml")
                addon_xml = ElementTree.parse(addon_xml_path)
                addon_root = addon_xml.getroot()
                adon_id = addon_root.get('id')
                version = addon_root.get('version')

                updated = False
                addon_entry = addons_root.find(addon_xpath.format(adon_id))
                ...
                if updated:
                    ...
                    self._create_zip(addon, adon_id, version)
                    self._copy_meta_files(
                        addon, os.path.join(self.zips_path, adon_id))
            except OSError:
                print(
                    "Excluding "+addon
                )
        ...
        if changed:
            addons_root[:] = sorted(
                addons_root, key=lambda addon: addon.get('id'))
            try:
                addons_xml.write(
                ...
                print(
                    "An error occurred updating " + addons_xml_path
                )
