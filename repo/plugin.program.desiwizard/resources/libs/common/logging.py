...
def upload_log():
    ...
    for item in files:
        ...
        if succes:
            ...
            succes, result = post_log_pastebin(content, name)
            if succes:
...
def post_log_pastebin(data, name):
    base_link = 'https://pastebin.com'
    paste_link = '/api/api_post.php'
    apiKey = base64.b64decode('SzIzaTN6dUQ0T0l2dldTVnlKMVBVa1VKVjN6elV5WV8=')
    url = urljoin(base_link, paste_link)
    payload = {'api_dev_key': apiKey, 'api_option':'paste', 'api_paste_code': data}
    result = ''
    ...
    try:
        if not PY2:
            page_url = result.decode('UTF-8')
            log("URL for {0}: {1}".format(name, page_url))
            return True, page_url
        if not base_link in result: 
             a = 'unable to retrieve the paste url'
             log("{0}: {1}".format(a, str(e)), level=xbmc.LOGERROR)
             return False, a
